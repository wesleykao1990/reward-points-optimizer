-- Staged from db/0025_osm_runtime_ingest_and_nearby_views.sql; edit the canonical source, not this file.
-- Runtime OSM discovery boundary for Nearby merchant search.
-- Collected locations and payment facts become active immediately, while the
-- SECURITY DEFINER function remains tightly constrained to merchant acceptance.
-- The final function includes the prewarmed-location match behavior that was
-- added as a follow-up production migration on 2026-08-23.
...
declare
  ...
  v_requested_location_key text;
  v_merchant_key text;
  v_merchant_id uuid;
  v_existing_location_merchant_id uuid;
  v_location_id uuid;
  v_location_key text;
  v_lat numeric;
  v_lon numeric;
...
begin
  ...
  loop
    ...
    v_requested_location_key := nullif(v_place ->> 'location_key', '');
    ...
    if v_requested_location_key is not null
       and v_requested_location_key !~ '^[a-z0-9][a-z0-9._-]{0,190}$' then
      raise exception 'osm_location_key_invalid' using errcode = '22023';
    end if;
    begin
    ...
    if v_requested_location_key is not null then
      select id, merchant_entity_id
        into v_location_id, v_existing_location_merchant_id
      from app_private.merchant_locations
      where location_key = v_requested_location_key;
      if v_location_id is null then
        raise exception 'osm_matched_location_not_found' using errcode = '22023';
      end if;
      if v_existing_location_merchant_id <> v_merchant_id then
        raise exception 'osm_matched_location_merchant_mismatch' using errcode = '22023';
      end if;
      v_location_key := v_requested_location_key;
    else
    ...
    on conflict (location_key) do update
      set display_name = case
            when app_private.merchant_locations.metadata ? 'launch_area'
              then app_private.merchant_locations.display_name
            else excluded.display_name
          end,
          address = case
            when app_private.merchant_locations.address is null
                 or app_private.merchant_locations.address = '{}'::jsonb
              then excluded.address
            else app_private.merchant_locations.address
          end,
          latitude = excluded.latitude,
          longitude = excluded.longitude,
          external_place_ids = app_private.merchant_locations.external_place_ids || excluded.external_place_ids,
          confidence = case
            when app_private.merchant_locations.confidence = 'official_directory'
              then app_private.merchant_locations.confidence
...
comment on function app_private.ingest_osm_nearby_snapshot(text, integer, timestamptz, timestamptz, jsonb) is 'Bounded jro_runtime ingest for OSM nearby discovery. Optional validated location_key lets a matched OSM branch enrich an existing prewarmed location instead of creating a duplicate; it cannot publish reward economics.';
