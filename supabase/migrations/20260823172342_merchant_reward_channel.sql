...
alter table app_private.merchant_tender_reward_facts
  add constraint merchant_tender_reward_facts_channel_check
  check (channel in ('in_store','mobile_order','matsuben','online','delivery'));
...
select
  ...
  component_kind,stacking_mode,choice_group,eligibility,reward_units,spend_jpy,tax_basis,
  channel
...
with candidates as (
 ...
 select c.*,
   row_number() over (
     partition by c.location_key,c.loyalty_program_key,coalesce(c.payment_instrument_key,''),c.component_kind,coalesce(c.choice_group,''),c.channel
