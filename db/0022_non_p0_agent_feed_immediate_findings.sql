...
create table if not exists user_data.agent_feed_finding_corrections (
    ...
    -- No `default auth.uid()` here: a column default is resolved when the
    -- table is created, and the `auth` schema only exists on the deployment
    -- platform, so the migration cannot be applied anywhere else.  The RPC
    -- below already reads auth.uid() from inside a function body, where it is
    -- resolved at call time, and supplies this column explicitly.
...
-- `anon` and `authenticated` are deployment-platform roles that do not exist in
-- plain PostgreSQL, so naming them directly in a grant or revoke aborts the
-- migration everywhere else.  db/0001 established the guarded form for exactly
-- this reason; these grants follow it, and are simply skipped when the roles
-- are absent.
do $platform_roles$
declare
    role_name text;
begin
    foreach role_name in array array['anon','authenticated'] loop
        if exists (select 1 from pg_roles where rolname = role_name) then
            execute format(
                'revoke all on app_private.agent_feed_experimental_findings from %I',
                role_name
            );
            execute format(
                'revoke all on user_data.agent_feed_finding_corrections from %I',
                role_name
            );
            execute format('grant usage on schema app_api to %I', role_name);
            execute format(
                'grant select on app_api.active_agent_feed_experimental_findings to %I',
                role_name
            );
        end if;
    end loop;

    -- Only a signed-in caller may file a correction, so this grant is not
    -- extended to `anon`.
    if exists (select 1 from pg_roles where rolname = 'authenticated') then
        execute
            'grant execute on function '
            || 'app_api.flag_agent_feed_experimental_finding(text,text,text) '
            || 'to authenticated';
    end if;
end
$platform_roles$;
