(() => {
  ...
  const exactCardLabels = Object.freeze({
    ...
    "instrument.card.marriott-bonvoy-american-express-premium-card": "Marriott Bonvoy アメリカン・エキスプレス・プレミアム・カード",
    "instrument.card.hilton-honors-american-express-card": "ヒルトン・オナーズ アメリカン・エキスプレス・カード",
    "instrument.card.hilton-honors-american-express-premium-card": "ヒルトン・オナーズ アメリカン・エキスプレス・プレミアム・カード",
  });
  ...
  const knownProvider = (issuer = "") => {
    if (/AEON/iu.test(issuer))
      return { key: "family:card.aeon", label: "イオンカード", engineFamily: "card.aeon", order: 10 };
    if (/au Financial/iu.test(issuer))
      return { key: "family:card.aupay", label: "au PAY カード", engineFamily: "card.aupay", order: 20 };
    if (/NTT DOCOMO/iu.test(issuer))
      return { key: "family:card.d", label: "dカード", engineFamily: "card.d", order: 30 };
    if (/PayPay/iu.test(issuer))
      return { key: "family:card.paypay", label: "PayPayカード", engineFamily: "card.paypay", order: 40 };
    if (/Rakuten/iu.test(issuer))
      return { key: "family:card.rakuten", label: "楽天カード", engineFamily: "card.rakuten", order: 50 };
    if (/Viewcard/iu.test(issuer))
      return { key: "family:card.view", label: "ビューカード", engineFamily: "card.view", order: 60 };
    if (/Sumitomo Mitsui Card/iu.test(issuer))
      return { key: "family:card.smbc", label: "三井住友カード", engineFamily: "card.smbc", order: 70 };
    if (/American Express/iu.test(issuer))
      return { key: "issuer:american-express", label: "American Express", engineFamily: null, order: 80 };
    const base = String(issuer || "その他").split(" / ")[0].trim() || "その他";
    return {
  ...
  const readSelectedSkus = () => {
    try {
      const value = JSON.parse(localStorage.getItem(CARD_SKU_STORAGE_KEY) || "[]");
  ...
  const writeSelectedSkus = (selected) => {
    try {
      localStorage.setItem(CARD_SKU_STORAGE_KEY, JSON.stringify([...selected].sort()));
    } catch {}
  };

  const cardLabel = (card) => exactCardLabels[card.item_id] || card.display_name;

  const manifestAsset = (id) => {
    const asset = state.manifestById.get(id);
    if (!asset) return null;
    if (String(asset.id).startsWith("card.") && String(asset.resolved_id).startsWith("instrument.card."))
  ...
  const setManifestAsset = (frame, id, cardLayout = false) => {
    ...
    if (frame.dataset.dbManifestPath === asset.path && frame.querySelector("img")) return;
  ...
  const normalizeServiceOptions = () => {
    document
      ...
      .forEach((option) => {
        ...
        if (strong) strong.textContent = serviceName(input.value, strong.textContent);
        const small = option.querySelector(".p0-product-name small");
        if (small && /^(?:ウォレット|ポイント|電子マネー|電子マネー残高|wallet|point)/iu.test(small.textContent || ""))
          small.remove();
        setManifestAsset(option.querySelector(".payment-logo"), input.value, false);
      });
  };
  ...
  const providerGroups = () => {
    ...
    return [...groups.values()]
      .map((group) => ({
        ...group,
        cards: group.cards.sort((a, b) => cardLabel(a).localeCompare(cardLabel(b), "ja")),
      }))
  ...
  const extractProxyHolder = (picker, selector) => {
    const inputs = [...picker.querySelectorAll(selector)].filter((input) => input.value?.startsWith("card."));
    const holder = document.createElement("div");
    holder.className = "db-card-proxies";
    holder.hidden = true;
    inputs.forEach((input) => holder.appendChild(input));
    return holder;
  };
  ...
  const defaultSkuForProvider = (provider) => {
    if (provider.engineFamily) {
      ...
      if (alias?.resolved_id && provider.cards.some((card) => card.item_id === alias.resolved_id))
  ...
  const renderProvider = (provider, proxy, selected, mode) => {
    ...
    const selectedCount = provider.cards.filter((card) => selected.has(card.item_id)).length;
    ...
    provider.cards.forEach((card) => {
      ...
      note.textContent = provider.engineFamily ? "このカードを選択" : "カタログ収録（最適化は準備中）";
      ...
      checkbox.addEventListener("change", () => {
        ...
        if (proxy) {
          const any = provider.cards.some((candidate) => next.has(candidate.item_id));
  ...
  const renderCardPicker = (picker, mode) => {
    if (!picker) return;
    const selector = mode === "earn" ? 'input[data-p0-product]' : 'input[data-payment-stack-owned="true"]';
    const proxies = proxyMap(picker, selector);
    const selected = readSelectedSkus();
    const groups = providerGroups();

    groups.forEach((provider) => {
      const proxy = provider.engineFamily ? proxies.get(provider.engineFamily) : null;
      if (proxy?.checked && !provider.cards.some((card) => selected.has(card.item_id))) {
    ...
    groups.forEach((provider) => {
      const proxy = provider.engineFamily ? proxies.get(provider.engineFamily) || null : null;
  ...
  const syncProxyChecks = () => {
    ...
    groups.forEach((provider) => {
      ...
      document
        .querySelectorAll(`input[data-p0-product][value="${provider.engineFamily}"], input[data-payment-stack-owned="true"][value="${provider.engineFamily}"]`)
        .forEach((input) => {
          input.checked = any;
        });
    });
  ...
  const enhance = () => {
    ...
    try {
      ...
      if (earnPicker && !earnPicker.querySelector(":scope > .db-card-catalogue"))
        renderCardPicker(earnPicker, "earn");
      const spendPicker = findSpendCardPicker();
      if (spendPicker && !spendPicker.querySelector(":scope > .db-card-catalogue"))
  ...
  const load = async () => {
    const [catalogueResponse, manifestResponse] = await Promise.all([
      fetch("/catalogue-ui", { headers: { Accept: "application/json" }, cache: "no-store" }),
      fetch("/assets/liquid-glass/manifest.json", { headers: { Accept: "application/json" }, cache: "no-store" }),
    ]);
    if (!catalogueResponse.ok || !manifestResponse.ok) throw new Error("catalogue_sync_unavailable");
    const catalogue = await catalogueResponse.json();
    const manifest = await manifestResponse.json();
    if (catalogue.version !== "consumer-catalogue-ui.v1" || !Array.isArray(catalogue.items))
      throw new Error("catalogue_sync_invalid");
    if (manifest.version !== "liquid-glass-assets.v2" || !Array.isArray(manifest.assets))
      throw new Error("manifest_invalid");
    state.catalogueById = new Map(catalogue.items.map((item) => [item.item_id, item]));
    state.cards = catalogue.items.filter((item) => item.item_kind === "credit_card");
    state.manifestById = new Map(manifest.assets.map((asset) => [asset.id, asset]));
    state.ready = true;
    scheduleEnhance();
  };
  ...
  document.getElementById("wallet-reset-button")?.addEventListener("click", () => {
    try { localStorage.removeItem(CARD_SKU_STORAGE_KEY); } catch {}
    scheduleEnhance();
  }, { capture: true });
